        ...
        for i in range(5):
            if enable_beacon_mode() == 0:
                ...
                if rssi_status is True:
                    tkinter.messagebox.showinfo('PASS', 'è‰¯å“' + str(rssi_val) + "dB")
                    print('PASS', 'è‰¯å“' + str(rssi_val) + "dB")
                else:
                    tkinter.messagebox.showinfo('å¤±è´¥', 'äº§å“ä¸è‰¯')
                    print("test fail!")

                # self.init_data_label = Label(self.init_window_name, text="[enable]é‡è¯•æ¬¡æ•°:" + str(0))
                # self.init_data_label.grid(row=30, column=0)
                break
            else:
                self.init_data_label = Label(self.init_window_name, text="[enable]é‡è¯•æ¬¡æ•°:" + str(i + 1))
                self.init_data_label.grid(row=30, column=0)
    ...
    def disable_beacon(self):
        for i in range(5):
            if disable_beacon_mode() == 0:
                # self.init_data_label = Label(self.init_window_name, text="[disable]é‡è¯•æ¬¡æ•°:" + str(0))
                # self.init_data_label.grid(row=60, column=0)
                break
            else:
                self.init_data_label = Label(self.init_window_name, text="[disable]é‡è¯•æ¬¡æ•°:" + str(i + 1))
                self.init_data_label.grid(row=60, column=0)
...
def log_init():
    ...
    #################################################################################################
    # å®šä¹‰ä¸€ä¸ªRotatingFileHandlerï¼Œæœ€å¤šå¤‡ä»½5ä¸ªæ—¥å¿—æ–‡ä»¶ï¼Œæ¯ä¸ªæ—¥å¿—æ–‡ä»¶æœ€å¤§10M
    Rthandler = RotatingFileHandler('myapp.log', maxBytes=10 * 1024 * 1024, backupCount=5)
    Rthandler.setLevel(logging.INFO)
    formatter = logging.Formatter('%(name)-12s: %(levelname)-8s %(message)s')
    Rthandler.setFormatter(formatter)
